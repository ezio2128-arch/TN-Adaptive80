...
void FidelityFX::CreateFSRResources()
{
	...
	auto& upscaling = globals::features::upscaling;
	if (upscaling.IsAdaptive80Active()) {
		// Reserve the legal output-sized maximum once. Adaptive 80 dispatches only
		// the active sub-rectangle, while later preset edits remain inside the FSR context bounds.
