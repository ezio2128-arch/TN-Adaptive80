...
// ============================================================================
// TN Smooth Motion Blur - Alpha 0.4 "Adaptive 10 FPS Rescue"
// Community Shaders 1.8.3 normal Nexus build.
// Goal: preserve Alpha 0.3 smoothness at ~20-30 base FPS, then add a conditional
// emergency integration path for severe ~10-15 FPS drops. Designed to feed CS FG.
// Uses velocityTex already bound to the temporal pass; no optical-flow pass.
// Runs before HUD/UI composition.
// ============================================================================
#ifndef TNMB_ENABLED
#define TNMB_ENABLED 1
#endif

// Low-FPS tuning. Motion-vector displacement itself acts as the FPS proxy:
// at the same camera speed, lower base FPS -> larger per-frame displacement ->
// stronger integration automatically.
#ifndef TNMB_MAX_BLUR_PIXELS
#define TNMB_MAX_BLUR_PIXELS 18.0
#endif
#ifndef TNMB_START_PIXELS
#define TNMB_START_PIXELS 0.35
#endif
#ifndef TNMB_FULL_PIXELS
#define TNMB_FULL_PIXELS 7.5
#endif
#ifndef TNMB_BASE_BLEND
#define TNMB_BASE_BLEND 0.34
#endif
#ifndef TNMB_LOW_FPS_EXTRA_BLEND
#define TNMB_LOW_FPS_EXTRA_BLEND 0.12
#endif
#ifndef TNMB_LOW_FPS_BOOST_START
#define TNMB_LOW_FPS_BOOST_START 4.5
#endif
#ifndef TNMB_LOW_FPS_BOOST_FULL
#define TNMB_LOW_FPS_BOOST_FULL 11.0
#endif
#ifndef TNMB_TAP1
#define TNMB_TAP1 0.18
#endif
#ifndef TNMB_TAP2
#define TNMB_TAP2 0.40
#endif
#ifndef TNMB_TAP3
#define TNMB_TAP3 0.70
#endif
#ifndef TNMB_RESCUE_START_PIXELS
#define TNMB_RESCUE_START_PIXELS 8.0
#endif
#ifndef TNMB_RESCUE_FULL_PIXELS
#define TNMB_RESCUE_FULL_PIXELS 18.0
#endif
#ifndef TNMB_RESCUE_EXTRA_BLEND
#define TNMB_RESCUE_EXTRA_BLEND 0.18
#endif
#ifndef TNMB_TAP4
#define TNMB_TAP4 0.86
#endif
#ifndef TNMB_TAP5
#define TNMB_TAP5 1.00
#endif

float2 TNMB_ClampMotionVector(float2 motionVector)
{
    float2 texel = max(abs(TexelOffset.xy), float2(1e-6, 1e-6));
    float speedPixels = length(motionVector / texel);
    if (speedPixels > TNMB_MAX_BLUR_PIXELS)
        motionVector *= TNMB_MAX_BLUR_PIXELS / max(speedPixels, 1e-4);
    return motionVector;
}

float TNMB_EdgeWeight(float3 centerColor, float3 sampleColor)
{
    // Cheap colour-edge rejection. It suppresses foreground/background bleeding
    // without extra depth texture reads, keeping the low-power GPU path light.
    float3 d = abs(sampleColor - centerColor);
    float maxDiff = max(d.x, max(d.y, d.z));
    return saturate(1.10 - maxDiff * 3.25);
}

float3 TNMB_ApplySmoothMotion(float3 temporalColor, float2 texCoord, float2 motionVector, float2 drMax)
{
#if TNMB_ENABLED
    float2 texel = max(abs(TexelOffset.xy), float2(1e-6, 1e-6));
    float speedPixels = length(motionVector / texel);

    // Keep stationary/very slow pixels untouched and avoid extra texture taps.
    if (speedPixels <= TNMB_START_PIXELS)
        return temporalColor;

    float response = smoothstep(TNMB_START_PIXELS, TNMB_FULL_PIXELS, speedPixels);
    float lowFpsProxy = smoothstep(TNMB_LOW_FPS_BOOST_START, TNMB_LOW_FPS_BOOST_FULL, speedPixels);
    float rescue = smoothstep(TNMB_RESCUE_START_PIXELS, TNMB_RESCUE_FULL_PIXELS, speedPixels);
    float amount = response * (TNMB_BASE_BLEND + TNMB_LOW_FPS_EXTRA_BLEND * lowFpsProxy);

    float2 mv = TNMB_ClampMotionVector(motionVector);

    // Alpha 0.3 core path: unchanged for ordinary 20-30 FPS motion.
    float2 uv1 = ClampScreenUV(texCoord + mv * TNMB_TAP1, drMax);
    float2 uv2 = ClampScreenUV(texCoord + mv * TNMB_TAP2, drMax);
    float2 uv3 = ClampScreenUV(texCoord + mv * TNMB_TAP3, drMax);
    float3 s1 = SampleCenterRGB(uv1);
    float3 s2 = SampleCenterRGB(uv2);
    float3 s3 = SampleCenterRGB(uv3);

    float w1 = 0.24 * TNMB_EdgeWeight(temporalColor, s1);
    float w2 = 0.16 * TNMB_EdgeWeight(temporalColor, s2);
    float w3 = 0.08 * TNMB_EdgeWeight(temporalColor, s3);
    float wc = 0.52;
    float invW = rcp(max(wc + w1 + w2 + w3, 1e-4));
    float3 integrated = (temporalColor * wc + s1 * w1 + s2 * w2 + s3 * w3) * invW;

    // Extreme-drop rescue. These two extra taps only execute for very large
    // per-frame displacement (roughly the regime seen during severe 10-15 FPS
    // dips at normal camera speed), so normal gameplay keeps Alpha 0.3's cost
    // and look. The original temporal result remains dominant to avoid smearing.
    if (rescue > 0.001)
    {
        float2 uv4 = ClampScreenUV(texCoord + mv * TNMB_TAP4, drMax);
        float2 uv5 = ClampScreenUV(texCoord + mv * TNMB_TAP5, drMax);
        float3 s4 = SampleCenterRGB(uv4);
        float3 s5 = SampleCenterRGB(uv5);

        float rw1 = 0.22 * TNMB_EdgeWeight(temporalColor, s1);
        float rw2 = 0.15 * TNMB_EdgeWeight(temporalColor, s2);
        float rw3 = 0.09 * TNMB_EdgeWeight(temporalColor, s3);
        float rw4 = 0.055 * TNMB_EdgeWeight(temporalColor, s4);
        float rw5 = 0.035 * TNMB_EdgeWeight(temporalColor, s5);
        float rwc = 0.45;
        float rinvW = rcp(max(rwc + rw1 + rw2 + rw3 + rw4 + rw5, 1e-4));
        float3 rescueIntegrated = (temporalColor * rwc + s1 * rw1 + s2 * rw2 + s3 * rw3 + s4 * rw4 + s5 * rw5) * rinvW;

        integrated = lerp(integrated, rescueIntegrated, rescue);
        amount += TNMB_RESCUE_EXTRA_BLEND * rescue;
    }

    return lerp(temporalColor, integrated, saturate(amount));
#else
    return temporalColor;
#endif
}
...
PS_OUTPUT main(PS_INPUT input)
{
	...
	float2 tnmbMotionVector = motionReject.xy;
...
#	endif
	...
	// TNMB Alpha 0.4: Alpha 0.3 low-FPS integration plus adaptive rescue for
	// severe frame drops. HUD/UI remains outside this pass.
	colorOut.xyz = TNMB_ApplySmoothMotion(colorOut.xyz, texCoord, tnmbMotionVector, drMax);
